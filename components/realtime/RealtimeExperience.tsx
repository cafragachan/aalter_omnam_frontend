...
// Phase A.2 walking skeleton: the full experience — UE5 twin (background iframe)
// + HeyGen LITE avatar (overlay) + gpt-realtime brain driving navigation via
// function calls. Dev HUD overlays status / latency / scene / transcript / log.
...
export default function RealtimeExperience() {
  ...
  const start = useCallback(async () => {
    ...
    session.setToolHandler(createToolDispatcher(ue5, { onScene: setScene }))
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000", overflow: "hidden", color: "#e7e9ee", fontFamily: "system-ui, sans-serif" }}>
      ...
      {/* Avatar (LITE) overlay — not chroma-keyed yet (polish later) */}
      <div style={{ position: "absolute", left: 16, bottom: 16, width: 300, height: 300, borderRadius: 12, overflow: "hidden", background: "#000", boxShadow: "0 8px 30px rgba(0,0,0,.5)", border: "1px solid #232838" }}>
        <video ref={videoRef} autoPlay playsInline style={{ width: "100%", height: "100%", objectFit: "cover" }} />
      </div>

      {/* Controls */}
      <div style={{ position: "absolute", top: 16, left: 16, display: "flex", gap: 10, alignItems: "center", background: "rgba(11,13,18,.8)", padding: "8px 12px", borderRadius: 10, backdropFilter: "blur(6px)" }}>
        {!active ? (
        ...
        <button onClick={() => setShowHud((v) => !v)} style={btn("#334155")}>{showHud ? "Hide HUD" : "HUD"}</button>
        {!ue5.isConnected && <span style={{ fontSize: 12, color: "#fbbf24" }}>UE5 ⚠ not connected</span>}
      </div>

      {/* HUD: transcript + event log */}
      {showHud && (
        <div style={{ position: "absolute", top: 16, right: 16, width: 360, maxHeight: "calc(100vh - 32px)", display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={card()}>
          ...
          <div style={{ ...card(), flex: 1, overflow: "auto", fontFamily: "ui-monospace, monospace", fontSize: 11 }}>
